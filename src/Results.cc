...
Results::Results(Projectile *proj, Recoil *rec, DetectorConstruction *det)
...
void Results::SetupRun() {
  ...
  for (i = 0; i < GN; i++)
    for (j = 0; j < GS; j++) {
      ...
      // %f\n",i,j,CP[i][j].getX(),CP[i][j].getY(),CP[i][j].getZ()); printf("%d
...
void Results::TreeCreate() {
  if (tree == NULL) {
    ...
    tree->Branch(
        "pHit", &pHit,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Id/I:ring/I:fold/I");
    tree->Branch(
        "rHit", &rHit,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Id/I:ring/I:fold/I");
    tree->Branch(
        "projGun", &gun,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "projBackingIn", &pBIn,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
    ...
    tree->Branch(
        ...
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "projDegraderOut", &pDOut,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "projDec", &pDec,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "recReactionOut", &rROut,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "recTargetOut", &rTOut,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        ...
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "recDegraderOut", &rDOut,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "recDegraderTrack", &rDTrack,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
    tree->Branch(
        "recDec", &rDec,
        "x/D:y/D:z/D:px/D:py/D:pz/D:E/D:b/D:w/D:path/D:Eloss/D:df/D:t/D");
...
void Results::FillTree(G4int evtNb, TrackerIonHitsCollection *IonCollection,
  ...
  //   printf("pos %2d index %2d kB %.4f kBm %.7f S
  ...
  if (Nt > 0) {
      ...
      case DEGRADER_IN_FLAG:
        ...
        if ((*IonCollection)[i]->GetA() == Ar)
          if ((*IonCollection)[i]->GetZ() == Zr) {
            ...
            rDiPos.set(rDIn.x, rDIn.y, rDIn.z); // set initial position when the
                                                // recoil enters the degrader
      ...
      case DEGRADER_TRACK_FLAG:
        if ((*IonCollection)[i]->GetA() == Ar)
          if ((*IonCollection)[i]->GetZ() == Zr) {
            ...
            rDPos.set(rDTrack.x, rDTrack.y,
                      rDTrack.z); // set current position when the recoil enters
                                  // the degrader
            rDTrack.path += CalculatePath(
                rDiPos, rDPos); // calculate the path step distance
      ...
      case DEGRADER_OUT_FLAG:
        ...
        if ((*IonCollection)[i]->GetA() == Ar)
          if ((*IonCollection)[i]->GetZ() == Zr) {
            ...
            rDPos.set(rDOut.x, rDOut.y,
                      rDOut.z); // set current position when the recoil leaves
                                // the degrader
            rDTrack.path += CalculatePath(
                rDiPos, rDPos);       // calculate the path step distance
      ...
      case REACTION_IN_FLAG:
        if ((*IonCollection)[i]->GetA() == Ap)
          if ((*IonCollection)[i]->GetZ() == Zp) {
            ...
            p1.set(pRIn.px, pRIn.py,
                   pRIn.pz); // projectile momentum vector before reaction
          }
        break;
      case REACTION_OUT_FLAG:
        if ((*IonCollection)[i]->GetA() == Ap)
          if ((*IonCollection)[i]->GetZ() == Zp) {
            ...
            p2.set(pROut.px, pROut.py,
                   pROut.pz); // projectile momentum vector after reaction
        ...
        if ((*IonCollection)[i]->GetA() == Ar)
          if ((*IonCollection)[i]->GetZ() == Zr) {
            ...
            p2.set(rROut.px, rROut.py,
                   rROut.pz); // recoil momentum vector after reaction
        ...
        if ((*IonCollection)[i]->GetA() == Ap)
          if ((*IonCollection)[i]->GetZ() == Zp) {
            if(decaySeen == false){
              ...
              //printf("i=%i Nt=%i px %f py %f pz %f\n",i, Nt, pDec.px, pDec.py, pDec.pz);
              //getc(stdin);
    ...
    // printf("DEGRADER: EIn %9.3f [MeV]  path %9.3f [mg/cm^2]  energy loss
    // %9.3f [MeV] EOut %9.3f [MeV] \n                               path %9.3f
    // [um]                                   check %8.3f
  ...
  if (Np > 0) {
    for (Int_t i = 0; i < Np; i++) {
      // G4cout<<"*"<<(*CsICollection)[i]->GetA()<<"
      // "<<(*CsICollection)[i]->GetZ()<<"
      // "<<(*CsICollection)[i]->GetKE()/MeV<<"
      // "<<(*CsICollection)[i]->GetId()<<"
      // "<<(*CsICollection)[i]->GetPos().getX()/um<<"
      // "<<(*CsICollection)[i]->GetPos().getY()/um<<"
      // "<<(*CsICollection)[i]->GetPos().getZ()/um<<G4endl;
      ...
      if ((*CsICollection)[i]->GetA() == Ar)
...

        {
          if (rHit.E == 0.) {
            ...
            // in mg/cm^2
            rHit.path =
                ((*CsICollection)[i]->GetPathLength() / cm) * CsIDensity * 1000;
            // dEdx=((*CsICollection)[i]->GetEdep()/MeV)/(((*CsICollection)[i]->GetPathLength()/cm)*CsIDensity*1000);

            // dL=CalculateBirksLawStep(rHit.Id,dE,dEdx); // make sure to check
            // units of kB! rHit.LY+=dL;

            // printf("RECOIL: id %d ring %d initial KE %9.3f MeV\nrecoil: dE
            // %9.3f path %9.3f  dE/dx %9.3f  dL %9.3f  LY
            // %9.3f\n",rHit.Id,rHit.ring,(*CsICollection)[i]->GetKE()/MeV,rHit.E,rHit.path,dEdx,dL,rHit.LY);

            // G4cout<<"+"<<i<<" "<<(*CsICollection)[i]->GetA()<<"
            // "<<(*CsICollection)[i]->GetZ()<<"
            // "<<(*CsICollection)[i]->GetEdep()/MeV<<"
            // "<<(*CsICollection)[i]->GetId()<<"
            // "<<(*CsICollection)[i]->GetPos().getX()/um<<"
            // "<<(*CsICollection)[i]->GetPos().getY()/um<<"
            // "<<(*CsICollection)[i]->GetPos().getZ()/um<<"
            // "<<(*CsICollection)[i]->GetPathLength()/um<<"
            // "<<((*CsICollection)[i]->GetEdep()/MeV)/((*CsICollection)[i]->GetPathLength()/um)<<G4endl;
            // getc(stdin);
            ...
            if ((*CsICollection)[i]->GetId() == rHit.Id) {
              ...
              // in mg/cm^2
              rHit.path += ((*CsICollection)[i]->GetPathLength() / cm) *
                           CsIDensity * 1000;
              // dEdx=((*CsICollection)[i]->GetEdep()/MeV)/(((*CsICollection)[i]->GetPathLength()/cm)*CsIDensity*1000);

              // dL=CalculateBirksLawStep(rHit.Id,dE,dEdx); // make sure to
              // check units of kB! rHit.LY+=dL;

              // printf("recoil: dE %9.3f path %9.3f  dE/dx %9.3f  dL %9.3f  LY
      ...
      if ((*CsICollection)[i]->GetA() == Ap)
        if ((*CsICollection)[i]->GetZ() == Zp) {
          if (pHit.E == 0.) {
            ...
            // dL=CalculateBirksLawStep(pHit.Id,dE,dEdx); // make sure to check
            // units of kB! pHit.LY+=dL;

            // printf("PROJECTILE: id %d ring %d\nprojectile: dE %9.3f path
            // %9.3f  dE/dx %9.3f  dL %9.3f  LY
            ...
            if ((*CsICollection)[i]->GetId() == pHit.Id) {
              ...
              // in mg/cm^2
              pHit.path += ((*CsICollection)[i]->GetPathLength() / cm) *
                           CsIDensity * 1000;
              // dEdx=((*CsICollection)[i]->GetEdep()/MeV)/(((*CsICollection)[i]->GetPathLength()/cm)*CsIDensity*1000);

              // dL=CalculateBirksLawStep(pHit.Id,dE,dEdx); // make sure to
              // check units of kB! pHit.LY+=dL;

              // printf("projectile: dE %9.3f path %9.3f  dE/dx %9.3f  dL %9.3f
    ...
    // printf("\nCsI recoil: fold %d id %d ring %d path: %9.3f [mg/cm^2] Energy:
    // %9.3f (%.3f) [MeV]  Light Output: %9.3f [arb.]\n            path: %9.3f
    // [um]",rHit.fold,rHit.Id,rHit.ring,rHit.path,rHit.E,rDOut.E,rHit.LY,rHit.path/CsIDensity*10);
    // printf("\nCsI projec: fold %d id: %d ring %d path: %9.3f [mg/cm^2]
    // Energy: %9.3f [MeV]  Light Output: %9.3f [arb.]  \n            path:
    // %9.3f
  ...
  for (i = 0; i < GN; i++)   // number of positions
    for (j = 0; j < GS; j++) // number of crystals
      if (gw[i][j] > 0.) {
        ...
        // printf("Detected: pos %d col %d ring %d energy %f weight
  ...
  for (i = 0; i < GN; i++) // number of positions
  {
    ...
    {
      if (gw[i][j] > 0.) {
        // printf("pos %d col %d energy %f weight
        // %f\n",i+1,j,ge[i][j],gw[i][j]);
        GHit.GEAB[GHit.GfoldAB] += ge[i][j];
        if (ge[i][j] > maxGe[i]) {
          GHit.GIdAB[GHit.GfoldAB] = i + 1;
          GHit.GSegAB[GHit.GfoldAB] = j;
          GHit.GRingAB[GHit.GfoldAB] =
              RingMap(GHit.GIdAB[GHit.GfoldAB],
                      GHit.GSegAB[GHit.GfoldAB]); // get the ring in which the
                                                  // hit occured
    ...
    if (GHit.GEAB[GHit.GfoldAB] > 0.) {
      // printf("Add back: pos %d col %d ring %d energy %f weight
  ...
  if (rHit.E > 0.)
    for (Int_t i = 0; i < GHit.GfoldAB; i++) {
      ...
      //cout << "Doppler factor: " << pDec.df << endl;
    }
...
void Results::ProjLabScattAngle() {
  ...
  if (h == NULL)
    h = new TH1D("Proj. Lab. Scatt. Angle", "Proj. Lab. Scatt. Angle", 180, 0,
                 180);
  h->Reset();

  g = (TH1D *)gROOT->FindObject("Rec. Lab. Scatt. Angle W");
  if (g == NULL)
    g = new TH1D("Proj. Lab. Scatt. Angle W", "Proj. Lab. Scatt. Angle W", 180,
                 0, 180);
  g->Reset();
...
void Results::RecLabScattAngle() {
  ...
  if (h == NULL)
    h = new TH1D("Rec. Lab. Scatt. Angle", "Rec. Lab. Scatt. Angle", 180, 0,
                 180);
  h->Reset();

  g = (TH1D *)gROOT->FindObject("Rec. Lab. Scatt. Angle W");
  if (g == NULL)
    g = new TH1D("Rec. Lab. Scatt. Angle W", "Rec. Lab. Scatt. Angle W", 180, 0,
                 180);
  g->Reset();
...
void Results::CrossSectionZProfile() {
  ...
  if (g == NULL)
    g = new TH1D("Cross Section vs. Z W", "Cross Section vs. Z W", 100, zmin,
                 zmax);
  g->Reset();
...
void Results::TargetFaceF_CM() {
  ...
  for (Int_t i = 0; i < N; i++) {
    ...
    // not really pi but transformation to CM only scales length, doesn't change
    // direction!
  ...
  for (i = 0; i < n; i++) {
    ...
    x[i] = (*th)[i];
    y[i] =
        norm * (*ff)[i] * sin((*th)[i] / RAD2DEG); // using new norm, see above
  }
...
void Results::PlotDistCloseApproach() {
  ...
  if (g == NULL)
    g = new TH1D("Distance of closest approach W",
                 "Distance of closest approach W", 200, 0, 20);
  g->Reset();
  ...
  for (Int_t i = 0; i < N; i++) {
    ...
    dc = 0.5 * AHC * (Double_t)Zp * (Double_t)Zr / EIn *
         (1 + (Double_t)Ap / (Double_t)Ar) * (1 + 1 / sin(0.5 * theta));
...
//=====================================================================================
void Results::DetRingCsIRingGammaSpectrum(G4int detRing, G4int pinRing,
                                          G4double coeff) {
  char name[132];
  G4double eg, ec;
  ...
  if (correlation == NULL)
    correlation =
        new TH2D("ECsI_EGamma", "ECsI_EGamma", 256, 0, 256, 2048, 0, 4096);
  correlation->Reset();
    ...
    if (rHit.ring == pinRing)
      for (Int_t j = 0; j < GHit.Gfold; j++)
        if (RingMap(GHit.GId[j], GHit.GSeg[j]) == detRing) {
          ...
          correlation->Fill(
              ec, eg, GHit.GW[j]); // should this have the CsI weight as well?
          h->Fill(eg, GHit.GW[j]);
        }
  }
  ...
  for (Int_t det = 0; det < GN; det++)
    for (Int_t cry = 0; cry < GS; cry++) {
      ...
      printf(" %2d  %2d  %3d %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n", det + 1,
             cry, RingMap(det + 1, cry), xx, yy, zz, rr, theta, phi);
    }
}
//=====================================================================================
...
void Results::GroupCosDist() {
  ...
  for (Int_t i = 0; i < N; i++) {
    ...
    if (rHit.E > 0) {
      ...
      for (Int_t j = 0; j < GHit.GfoldAB; j++) {
        // printf("Gamma-ray detected at pos %2d col
        // %d\n",GHit.GIdAB[j],GHit.GSegAB[j]);
        cos =
            PP[(Int_t)rHit.Id - 1].dot(CP[GHit.GIdAB[j] - 1][GHit.GSegAB[j]]) /
            PP[(Int_t)rHit.Id - 1].mag() /
            CP[GHit.GIdAB[j] - 1][GHit.GSegAB[j]].mag();
        h->Fill(cos);
        // printf("cos %9.5f\n",cos);
      }
    }
...
G4double Results::CalculateDistCloseApproach(G4double KEIn, G4double theta) {
  G4double d = 0.;
  d = 0.5 * AHC * (Double_t)Zp * (Double_t)Zr / KEIn *
      (1 + (Double_t)Ap / (Double_t)Ar) * (1 + 1 / sin(0.5 * theta));
  return d;
}

//=====================================================================================
G4double Results::CalculateDopplerFactor(IonInf Proj, G4double gx, G4double gy,
                                         G4double gz) {
  G4double df = 0.;
  G4ThreeVector ProjR, GammaR;  // positions
  G4ThreeVector eProjp, eGamma; // unit vectors for proj momentum and gamma dir
  G4double epsp = 0.000001; // small projectile momentum epsilon = 1 eV to avoid
                            // rounding errors
  ...
  if (df != df) {
    ...
    printf("Gamma.x %f Gamma.y %f Gamma.z %f Gamma.mag %f\n", GammaR.getX(),
           GammaR.getY(), GammaR.getZ(), GammaR.mag());
    printf("eGamma.x %f eGamma.y %f eGamma.z %f eGamma.mag %f\n", eGamma.getX(),
           eGamma.getY(), eGamma.getZ(), eGamma.mag());
    printf("beta %f eProjp.eGamma %f theta %f\n", Proj.b, eProjp.dot(eGamma),
           RAD2DEG * acos(eProjp.dot(eGamma)));
    printf("-----> df %f <-----\n", df);
    exit(EXIT_FAILURE);
  }

  // printf("Parameters:\neProjp.x %f eProjp.y %f eProjp.z %f eProjp.mag
  // %f\n",eProjp.getX(),eProjp.getY(),eProjp.getZ(),eProjp.mag());
  // printf("iProj.px %f iProj.py %f iProj.pz %f iProj.mag
  // %f\n",Proj.px,Proj.py,Proj.pz,mag); printf("iProj.x %f iProj.y %f iProj.z
  // %f\n",Proj.x,Proj.y,Proj.z); printf("Gamma.x %f Gamma.y %f Gamma.z %f
  // Gamma.mag %f\n",GammaR.getX(),GammaR.getY(),GammaR.getZ(),GammaR.mag());
  // printf("eGamma.x %f eGamma.y %f eGamma.z %f eGamma.mag
  // %f\n",eGamma.getX(),eGamma.getY(),eGamma.getZ(),eGamma.mag()); printf("beta
  // %f eProjp.eGamma %f theta
    ...
    for (j = 0; j < GS; j++)
      for (k = 0; k < NCsI; k++) {
        ...
        if (counter[i][j][k] > 0.) {
          ...
          if (dopplerFactor[i][j][k] > 1.011 &&
              dopplerFactor[i][j][k] <= 1.024) {
            group = 2;
            fprintf(group_map, "%2d     %d     %2d     %d\n", i + 1, j, k + 1,
                    group);
          }
          if (dopplerFactor[i][j][k] > 0.987 &&
              dopplerFactor[i][j][k] <= 1.011) {
            group = 3;
            fprintf(group_map, "%2d     %d     %2d     %d\n", i + 1, j, k + 1,
                    group);
          }
          if (dopplerFactor[i][j][k] > 0.976 &&
              dopplerFactor[i][j][k] <= 0.987) {
            group = 4;
            fprintf(group_map, "%2d     %d     %2d     %d\n", i + 1, j, k + 1,
                    group);
          }
