...
void TrackerIonSD::Initialize(G4HCofThisEvent *) {
  ...
  ionCollection =
      new TrackerIonHitsCollection(SensitiveDetectorName, collectionName[0]);
}
//--------------------------------------------------------------------
...
void TrackerIonSD::EndOfEvent(G4HCofThisEvent *HCE) {
  ...
  if (NbHits > 0) {
    ...
    for (i = 0; i < NbHits - 1; i++) {
      ...
      if ((*ionCollection)[i]->GetVolName() == "expHall" &&
          (*ionCollection)[i + 1]->GetVolName() == "degrader")
        (*ionCollection)[i + 1]->SetDegraderInFlag();
      if ((*ionCollection)[i]->GetVolName() == "degrader" &&
          (*ionCollection)[i + 1]->GetVolName() == "degrader")
        (*ionCollection)[i + 1]->SetDegraderTrackFlag();
    }
    ...
    for (i = 1; i < NbHits; i++) {
      ...
      if ((*ionCollection)[i - 1]->GetVolName() == "target" &&
          (*ionCollection)[i]->GetVolName() == "backing")
        (*ionCollection)[i]->SetTargetOutFlag();
    ...
    // for (i=1;i<NbHits;i++)
    // 	 {
    // 	   G4double pCheck = (*ionCollection)[i]->GetMom().mag();
    // 	   if (pCheck != pCheck)
    // 	     {
    // 	       G4cout<<G4endl;
    // 	       G4cout << "-------->ERROR!! Momentum is nan! "<<G4endl;
    ...
    if (print) {
      ...
      G4cout << " F  A  Z  KE/MeV   beta"
             ...
             << " " << std::setw(8) << std::right << "weight" << G4endl;
